...
- **后端测试**：157 passed（覆盖率 82%）。
- **下一步**：A8 采购缺口去重 —— 见 `BACKLOG.md`。
...
1. **A8 采购缺口去重**（与 I8 双来源相关）。
2. **i18n Phase 3 剩余**：后端错误串本地化（前端目前靠 `.includes('超出计划范围')` 匹配中文）、种子食材/分类名双语。用户自建食材名、AI 生成正文**保持创建语言**（设计边界，不改）。
3. **B4.4** AI 新食材：命中已有食材但与 AI 估算差距大时，换食材或换菜（阶段 B 未做）。
4. **Phase 2**：E1 食材多单位、E2 USDA API 补营养。
