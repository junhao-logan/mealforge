...
- **后端测试**：137 passed（覆盖率 81%）。
- **下一步**：A5 删除餐次退库存 / A6 零数量批次规则，先谈设计 —— 见 `BACKLOG.md`。
...
1. **A5 / A6**（删除餐次退库存、零数量批次规则）—— A4 已完成，这两条与 I1 相关，先谈设计。
2. **A4 遗留两处不一致**：采购缺口仍把过期库存算作「有」（库存页会提示缺、采购页不提示）；「今天」用服务器 UTC 日期（纽约晚 8 点后当天到期的批次会被当成过期）。
3. **A8 采购缺口去重**（与 I8 双来源相关）。
4. **i18n Phase 3 剩余**：后端错误串本地化（前端目前靠 `.includes('超出计划范围')` 匹配中文）、种子食材/分类名双语。用户自建食材名、AI 生成正文**保持创建语言**（设计边界，不改）。
5. **B4.4** AI 新食材：命中已有食材但与 AI 估算差距大时，换食材或换菜（阶段 B 未做）。
6. **Phase 2**：E1 食材多单位、E2 USDA API 补营养。
