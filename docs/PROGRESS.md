...
- **后端测试**：147 passed（覆盖率 81%）。
- **下一步**：A4.1 / A4.2（统一过期口径、「今天」改用本地日期）—— 见 `BACKLOG.md`。
...
1. **A4 遗留两处不一致**（A4.1 / A4.2）：采购缺口仍把过期库存算作「有」（库存页会提示缺、采购页不提示）；「今天」用服务器 UTC 日期（纽约晚 8 点后当天到期的批次会被当成过期）。
