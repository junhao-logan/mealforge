...
## Week 12 — A4 库存预留视图 + 手选批次（2026-09-23 ~ 09-24）

**完成**：
- 库存页按「食材 + 储存区」合并卡片，展开看每批被哪顿饭预留（未预留在前/在后可切换），顶部缺口提示
- 餐次弹窗：这顿饭全部食材及来源批次；可手选批次（只能选未被预留的余量，按顺序取够，过期批次只能手选）
- 后端 `GET /inventory/reservations` 读时两阶段分配；`PUT .../entries/{id}/picks`；新表 `entry_batch_picks`（迁移 `a4b7c2d9e813`，往返 + `alembic check` 通过）
- `deduct_for_entry` 与预留同规则：先手选、再 FEFO 未过期、让开别人手选的量
- 顺带修库存页食材名只取前 20 条的 bug；编辑弹窗按规范单位显示
- 测试 116 → **137**，覆盖率 81%

**关键决策**：I14（自动预留读时算、手选持久化；显示 = 扣减），修订 I6 / I1

**踩坑**：规则改成「自动不用过期批次」后，原 FEFO 测试写死的 1 月过期日全变成过期 → 改为相对今天的未来日期

**遗留**：I7 采购缺口仍计入过期库存；「今天」用服务器 UTC 日期
...
## Week 12 — A5 删除时退回库存 + A6 零数量批次隐藏（2026-09-24）

**完成**：
- 删除已完成的餐次：弹窗三选一（取消 / 只删记录 / 删除并退回库存），餐计划页与今日概览共用 `useEntryDelete`
- 删除计划：有已完成餐次时统一问一次是否退回
- 退不回去的部分（原批次已删）：`reversal_lost` 冲销流水 + 操作后提示；撤销完成同样提示
- 用到 0 的批次默认不在 `GET /inventory` 返回（`?include_empty=true` 可查看），数据库保留
- 补缺失翻译 key `mealPlans.errPlanName`
- 测试 137 → **147**，覆盖率 81%

**关键决策**：I15（删除时由用户选是否退回；退回在删除前同事务；零数量批次保留但隐藏）

**踩坑**：退回必须在删除之前 —— 删除后流水的 `source_entry_id` 被置空，就找不到扣过哪些批次
...
## Week 12 — A4.2「今天」按用户时区 + A4.1 采购不算过期库存（2026-09-24）

**完成**：
- 前端每个请求带 `X-Timezone`；后端 `get_today` 依赖统一算当地日期，无效退回 UTC；原 8 处 `date.today()` 全部替换
- 新增 `tzdata` 依赖（精简镜像无系统时区数据），`uv.lock` 只 +11 行
- 采购缺口 / 预扣视图的现有库存排除过期批次，与库存预留口径一致
- 测试 147 → **157**，覆盖率 82%；CORS 预检带 `x-timezone` 验证通过

**关键决策**：I16（请求头时区 vs 存用户资料；tzdata 兜底；采购口径统一到「不算过期」）

**踩坑**：同一测试文件混排同步 / 异步测试时，模块级 `pytestmark = asyncio` 会对同步测试报警告 → 异步测试逐个标记（Week 9 教训再次出现）
...
## Week 12 — A8 采购去重：在途量（2026-09-24）

**完成**：
- `in_flight_by_ingredient`：所有进行中清单里未购入库项按食材合计
- 生成 / 重算清单扣在途量；缺口预览返回已在清单 / 还需加；已安排够的显示「已在清单中」
- 加入清单并入同清单已有 manual 行；结算关掉同食材兄弟行，只入库一批
- 清单同食材 auto + manual 显示合并一行（`shoppingView.groupListItems`）
- 采购页按规范单位显示与结算（修单位本位食材结算 422）；食材名 / 单位随条目返回
- 测试 157 → **169**，覆盖率 82%
