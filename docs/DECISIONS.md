...
- **扣减顺序（FEFO）**：`expires_at ASC NULLS LAST, purchased_at ASC NULLS LAST, id ASC`
  （2026-09-24 修订，见 I14：自动扣减跳过已过期批次；手选批次优先）
...
- **零余量批次保留**（不删除；2026-09-24 起列表默认隐藏，见 I15）：撤销完成餐次时可直接 `+= take` 回补，
...
### I6 — 库存预扣视图 ✅ Week 6 实现（2026-09-24 部分修订：手选批次持久化，见 I14）
...
### I14 — 库存预留视图 + 手选批次（A4）✅ Week 12 实现（修订 I6）

**需求**：库存里区分「已排未做」占用的量；看得到每批被哪顿饭预留；用户能指定某顿饭用哪一批。

- **两阶段分配**（`app/inventory/reservations.py`）：
  1. **手选 = 硬预留**：用户为某餐某食材勾选的批次，按勾选顺序取够；可以选过期批次
  2. **自动 = 软预留**：剩余需求按餐次先后（日期 → 早/午/晚 → sort_order → id）走 FEFO，**只用未过期批次**；不够记为缺口
- **对 I6 的修订**：I6 原则「预扣是读时视图、不落库」**对自动部分保持不变**；手选是用户的决定，必须持久化 → 新表 `entry_batch_picks(entry_id, ingredient_id, inventory_item_id, position)`。
  只存「选了哪几批、什么顺序」，不存取量 —— 取量每次按规则现算，批次余量变了也不会存出矛盾的数字
- **显示 = 扣减**：`deduct_for_entry` 改为同一规则（先手选、再 FEFO 未过期、让开其他餐的手选量）。
  两边共用 `fefo_key` / `is_expired` / `line_demand`，有测试互相印证。按日期顺序做饭时，模拟与真实扣减完全一致
- **可选范围**：只能选「未被其他餐预留的余量 + 本餐当前占的」，后端同口径校验（400）。手选从未预留的量里取，所以不会抢走别人的分配
- **窗口**：今天及以后所有未完成餐次；已过日期没做的不占库存；已完成的已扣过库存，不单独显示
- **生命周期**：餐次删 / 批次删 → picks CASCADE；已完成餐次不能改手选；撤销完成后手选重新生效
- **备选（否决）**：
  - 全部预留落库（真预留）—— 计划一改就要回滚，正是 I6 当初否决的理由
  - 只改显示、扣减不变 —— 页面显示和实际扣的对不上
- **行为变化**：完成餐次时自动扣减不再动过期批次（以前会）；不够即短缺
- **已知遗留**：~~I7 采购缺口仍把过期库存计入「有」；「今天」按服务器 UTC 日期~~ → 已由 I16 解决

### I15 — 删除时退回库存（A5）+ 零数量批次隐藏（A6）✅ Week 12 实现

**A5 删除已完成的餐次 / 计划**
- **由用户决定**：已完成的餐次删除时弹窗问「只删记录（饭确实吃了）」还是「删除并退回库存（其实没做）」；计划里有已完成餐次时统一问一次。
  理由：删除的意图两种都常见，系统猜哪一种都会错一半
- **接口**：`DELETE .../entries/{id}?restock=true`、`DELETE /meal-plans/{id}?restock=true`，默认不退（与旧行为一致）
- **顺序**：退回必须在删除**之前**、同一事务内 —— 删除后流水的 `source_entry_id` 被 SET NULL，就不知道这顿饭扣了哪些批次了
- **复用 I2 的净额回补**（`restock_for_entry`），撤销完成与删除走同一条路径
- **退不回去**（原批次已被手动删除）：不新建批次（流水里没有过期日 / 储存区，建出来的是残缺数据），
  记一条 `reason='reversal_lost'` 的冲销流水让净额归零 —— 否则反复「完成 → 撤销」会重复提示同一笔；
  库存数量不动，审计里留痕。响应返回 `unrestorable`，前端提示用户
- **接口变化**：删除从 204 改 200（带 `unrestorable`）；撤销完成返回 `{entry, unrestorable}`

**A6 用到 0 的批次**
- **保留在数据库，界面隐藏**：I1「零余量批次保留」不变 —— 退回原批次（A5 / 撤销完成）要靠它，才能保留原过期日和储存区
- `GET /inventory` 默认过滤 `quantity > 0`，`?include_empty=true` 可查看
- **备选（否决）**：
  - 归零即删 —— 退回时原批次已不在，只能建缺信息的新批次
  - 定期清理 —— 需要后台任务，且超过期限就退不回；当前数据量下空间成本可忽略（I1 估算约 150 KB/年），不值得

### I16 —「今天」按用户时区（A4.2）+ 采购不算过期库存（A4.1）✅ Week 12 实现

**A4.2 时区**
- **问题**：服务器（Fly）跑 UTC，所有「今天」都是 UTC 日期。纽约晚 8 点后 UTC 已是次日 →
  当天到期的批次被判过期（I14 自动分配跳过它）、当天晚餐被当成「已过日期」不占预留
- **决策**：前端每个请求带 `X-Timezone`（`Intl.DateTimeFormat().resolvedOptions().timeZone`）；
  后端 `get_today` 依赖（`app/core/dates.py`）按它算当地日期，**缺失 / 无效 / 超长一律退回 UTC**，不报错
- **备选（否决）**：存进用户资料（users.timezone）—— 要迁移 + 设置界面；换设备 / 出差不会自动跟着走。
  将来若做定时任务（如每天早上的临期提醒）需要服务端知道用户时区时，再加这一列
- **覆盖范围**：原 8 处 `date.today()` 全部改为依赖注入；service 层保留 `today=None` 参数（默认当天），便于测试注入固定日期
- **tzdata**：`zoneinfo` 依赖系统时区数据，新版 `python:3.12-slim`（Debian trixie）不再自带 → 加 `tzdata` 包兜底，
  否则线上会静默退回 UTC。只 `uv add` 这一个包，不连带升级其他依赖（DEP0 教训）
- **CORS**：`allow_headers=["*"]` 在带凭据时由 Starlette 回显请求头，`x-timezone` 预检已验证通过

**A4.1 采购口径**
- I7 采购缺口 / I6 预扣视图的「现有库存」**排除已过期批次**（当天到期仍算），与 I14「自动分配不用过期批次」一致
- **已知小差异（接受）**：用户手选过期批次给某餐时，库存页不缺、采购页仍建议买 —— 完全统一需让采购改用 I14 预留引擎，改动大、收益小，暂不做

### I17 — 采购去重：在途量（A8）✅ Week 12 实现

- **问题**：I7 缺口只看「需求 − 库存」，不知道哪些已经在清单上等着买 → 多张清单、缺口页反复加入、重算叠加手动项，都会重复采购
- **在途量**（on-order quantity）：所有 `status='active'` 清单里 `is_purchased=false` 且 `add_to_inventory=true`、有食材与数量的条目，按食材合计。
  已购的会回流入库，缺口自然减少，不算在途；纯文本项 / 不入库项与库存缺口无关，不算
- **扣减位置**：I7 `compute_shortfall` **保持纯粹**（需求 − 可用库存），在途量在使用处扣：
  - 生成 / 重算清单：auto 需求 = 缺口 − 在途量（重算时本清单未购 auto 已先删掉，本清单 manual 计入在途）
  - 缺口预览：返回 `in_list_grams` 与 `to_add_grams = max(0, 缺口 − 在途)`
- **加入清单并入**：同清单已有该食材未购 manual 入库行 → 加量不新建。**不并入 auto 行** —— 重算删未购 auto 时并进去的量会丢
- **显示合并 + 结算关兄弟行**：界面把同清单同食材的 auto + manual 合成一行；结算代表行时后端把兄弟行一并标记已购（`close_sibling_items`），
  购买量与入库批次只记在代表行 —— 与显示一致，不重复入库
- **保留 I8**：auto / manual 仍是独立行（重算只动 auto），不改表结构、不需要迁移
- **备选（否决）**：
  - 只在单张清单内去重 —— 多清单并存时仍重复
  - 同时只允许一张进行中清单 —— 限制用法（如按超市分清单）
- **已知取舍**：别的清单里的 auto 是生成时的快照，可能已过时；在途量照单全收（宁可少买一点，下次重算会补）
